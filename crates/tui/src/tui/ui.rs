...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    if use_alt_screen {
        ...
        // Windows also suppresses Ghosty Code's own verbose CLI logger while
...
pub(crate) fn apply_engine_error_to_app(
    ...
    ) && app.api_key_env_only
    {
        ...
        app.onboarding = OnboardingState::ApiKey;
        app.status_message = Some(
            "The API key from DEEPSEEK_API_KEY was rejected. Paste a valid key to save it to ~/.ghosty/config.toml, or update the environment variable.".to_string(),
        );
        return;
    }
    if !recoverable {
...
async fn version_hint_from_release_mirror_env(current: &str) -> Option<String> {
    ...
    let tag = ghosty_release::latest_release_tag_async(ghosty_release::ReleaseChannel::Stable)
        .await
        .ok()?;
...
fn version_hint_from_latest_tag(tag: &str, current: &str) -> Option<String> {
    ...
    Some(format!(
        "v{latest} available - run `ghosty update` and restart"
    ))
}
